...
$agentes = $conn->query("SELECT * FROM agentes ORDER BY nombre")->fetch_all(MYSQLI_ASSOC);
$closers = $conn->query("SELECT * FROM closers ORDER BY nombre")->fetch_all(MYSQLI_ASSOC);

$stmt = $conn->prepare("SELECT * FROM datos_financieros WHERE mes = ? AND anio = ?");
$stmt->bind_param("ii", $mes_actual, $anio_actual);
$stmt->execute();
$datos_financieros_actuales = $stmt->get_result()->fetch_assoc() ?? [];
$stmt = $conn->prepare("SELECT * FROM metas_generales WHERE mes = ? AND anio = ?");
$stmt->bind_param("ii", $mes_actual, $anio_actual);
$stmt->execute();
            ...
            <form method="GET" action="index.php" class="row g-3 align-items-end">
                ...
                <div class="col-md-4">
                    ...
                    <select name="mes" id="mes" class="form-select">
                        <?php for ($i = 1; $i <= 12; $i++): ?>
                            <option value="<?php echo $i; ?>" <?php echo ($i == $mes_actual) ? 'selected' : ''; ?>><?php echo DateTime::createFromFormat('!m', $i)->format('F'); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div class="col-md-4">
                    <label for="anio" class="form-label">Año</label>
                    <input type="number" name="anio" id="anio" class="form-control" value="<?php echo $anio_actual; ?>">
                </div>
                <div class="col-md-4">
                ...
                <div class="card-body">
                    <form class="data-form" data-form-type="financiero">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" name="facturacion_cobrada" value="<?php echo $datos_financieros_actuales['facturacion_cobrada'] ?? ''; ?>">
                        </div>
                        <div class="mb-3">
                            <label for="facturas_cobradas" class="form-label">Facturas Cobradas (#)</label>
                            <input type="number" class="form-control" name="facturas_cobradas" value="<?php echo $datos_financieros_actuales['facturas_cobradas'] ?? ''; ?>">
                        </div>
                        <hr>
                        <h6 class="text-muted">Metas del Mes</h6>
                        <div class="mb-3">
                            <label for="meta_instalaciones" class="form-label">Meta de Instalaciones</label>
                            <input type="number" class="form-control" name="meta_instalaciones" value="<?php echo $metas_actuales['meta_instalaciones'] ?? ''; ?>">
                        </div>
                        <div class="mb-3">
                            <label for="meta_clientes" class="form-label">Meta de Clientes (Total)</label>
                            <input type="number" class="form-control" name="meta_clientes" value="<?php echo $metas_actuales['meta_clientes'] ?? ''; ?>">
                        </div>
                ...
                <div class="card-body" style="max-height: 400px; overflow-y: auto;">
                    <form class="data-form" data-form-type="sedes">
                        ...
                        <?php 
                            ...
                            $stmt = $conn->prepare("SELECT sede_id, instalaciones FROM rendimiento_sedes WHERE mes = ? AND anio = ?");
                            $stmt->bind_param("ii", $mes_actual, $anio_actual);
                            $stmt->execute();
                        ...
                        foreach ($sedes as $sede): ?>
                            <div class="input-group mb-2">
                                ...
                                <input type="number" class="form-control" name="sedes[<?php echo $sede['id']; ?>]" value="<?php echo $rendimiento_sedes_raw[$sede['id']] ?? ''; ?>">
                            </div>
                        <?php endforeach; ?>
                    ...
                    <form class="data-form" data-form-type="agentes">
                        ...
                        <table class="table table-sm">
                            ...
                            <?php 
                                ...
                                $stmt = $conn->prepare("SELECT agente_id, cierres, prospectos, meta_cierres, meta_prospectos FROM rendimiento_agentes WHERE mes = ? AND anio = ?");
                                $stmt->bind_param("ii", $mes_actual, $anio_actual);
                                $stmt->execute();
                                $result = $stmt->get_result();
                                while ($row = $result->fetch_assoc()) {
                            ...
                            foreach ($agentes as $agente): ?>
                                <tr>
                                    ...
                                    <td><input type="number" class="form-control form-control-sm" name="agentes[<?php echo $agente['id']; ?>][cierres]" value="<?php echo $rendimiento_agentes_raw[$agente['id']]['cierres'] ?? ''; ?>"></td>
                                    <td><input type="number" class="form-control form-control-sm" name="agentes[<?php echo $agente['id']; ?>][prospectos]" value="<?php echo $rendimiento_agentes_raw[$agente['id']]['prospectos'] ?? ''; ?>"></td>
                                    <td><input type="number" class="form-control form-control-sm" name="agentes[<?php echo $agente['id']; ?>][meta_cierres]" value="<?php echo $rendimiento_agentes_raw[$agente['id']]['meta_cierres'] ?? ''; ?>"></td>
                                    <td><input type="number" class="form-control form-control-sm" name="agentes[<?php echo $agente['id']; ?>][meta_prospectos]" value="<?php echo $rendimiento_agentes_raw[$agente['id']]['meta_prospectos'] ?? ''; ?>"></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                        </table>
                        ...
                        <input type="hidden" name="anio" value="<?php echo $anio_actual; ?>">
                         <table class="table table-sm">
                            ...
                            <?php 
                                ...
                                $stmt = $conn->prepare("SELECT closer_id, cierres, meta_cierres FROM rendimiento_closers WHERE mes = ? AND anio = ?");
                                $stmt->bind_param("ii", $mes_actual, $anio_actual);
                                $stmt->execute();
                                $result = $stmt->get_result();
                                while ($row = $result->fetch_assoc()) {
                            ...
                            foreach ($closers as $closer): ?>
                                <tr>
                                    ...
                                    <td><input type="number" class="form-control form-control-sm" name="closers[<?php echo $closer['id']; ?>][cierres]" value="<?php echo $rendimiento_closers_raw[$closer['id']]['cierres'] ?? ''; ?>"></td>
                                    <td><input type="number" class="form-control form-control-sm" name="closers[<?php echo $closer['id']; ?>][meta_cierres]" value="<?php echo $rendimiento_closers_raw[$closer['id']]['meta_cierres'] ?? ''; ?>"></td>
                                </tr>
                            <?php endforeach; ?>
                            </tbody>
                        </table>
